...
###TBD###
as_array(t$to(torch_device("cpu"), dtype = torch_double()))
```
...
Indexing and slicing tensors is 1-based and works like you'd expect from R:
...
dimension. `torch.unsqueeze()` adds a dimension of size `1` at a place specified by `dim`:
...
data to any dimensionality, provided it matches the number of elements in the tensor. Here we have a `3x2` tensor that is
reshaped to size `2x3`:
...
Like we said above, `view()` does not make copies, or put differently: The output tensor shares storage with the input tensor.
To verify:
...
These tensors, in torch terminology, are not "contiguous". [^1] One way to reshape them is use `contiguous()` on them before:

[^1]: Although it may sound like, "contiguous" does not correspond to what we'd call "contiguous in memory" in casual
    language.
...
However, under certain conditions, one or both tensors may be virtually expanded to have both tensors match up. This behavior
is called *broadcasting*, and is identical to that of Python's NumPy.

The rules are

1.  We align array shapes, *starting from the right*. E.g.:
