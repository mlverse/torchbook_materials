...
```{python}
l$parameters$weight$grad
l$parameters$bias$grad
```
...
nodes, in a unidirectional fashion, then `torch.nn.Sequential` can be used to build a simple graph.
...
Now let's see how using `Sequential` can simplify our example network. As an act of further simplication, we won't calculate
mean squared error "by hand" anymore either; instead, we'll use one of the loss functions torch provides, `torch.nn.MSELoss`.
