...
\begin{equation*}
\\nabla\_{\\hat{\\boldsymbol{\\beta}}} \\mathcal{L} = 2 \\mathbf{X}\^t (\\mathbf{X} \\hat{\\boldsymbol{\\beta}} - \\mathbf{y})
(\\\#eq:gradlosswrtbeta)
\\end{equation\*}
...
-   We update the weights, subtracting *learning\_rate* times the gradient, and proceed to the next iteration.
