...
The generator is given a random noise vector (1d), and has to produce images (2d, of a given resolution). Its main tool is
repeated application of *transposed convolutions* that upsample from a resolution of 1x1 to the required resolution of 28x28.

Following the DCGAN paper, the generator's `ConvTranspose2d` and `BatchNorm2d` layers are initialized according to a normal
...
While generator and discriminator have each their own losses, mathematically both use the same calculation, namely, binary
crossentropy:
...
First, the discriminator is trained. This is a two-stage procedure: - In stage (1), it is given the real images, together with
...
if all images are characterized as fake.

Once gradients have been obtained in both computations, the discriminator's weights are updated. Then it's the generator's
...
```{r, fig.asp=1, fig.width=8}
# dim 64 x 28 x 28 because drop 
images <- py$new[,1,,] 
images[images < 0] <- 0

par(mfrow = c(8,8), mar = rep(0.2, 4))
images %>%
  purrr::array_tree(1) %>%
  purrr::map(as.raster) %>%
  purrr::iwalk(~{plot(.x)})
...
As well as find out what proportion of real images was classified as real by the discriminator (green line), contrasting this
...
(and thus, close to each other), which obviously isn't the case here. This means there is still room for improvement.
