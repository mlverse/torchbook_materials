...
In this chapter, we look at the other -- as of this writing -- main type of architecture used for unsupervised learning:
variational autoencoders (VAEs). VAEs are autoencoders, in that they compress their input and, starting from a compressed
representation, aim for a faithful reconstruction. But in addition, there is constraint on that compressed representation: It
is regularized so as to deviate as little as possible from a prior distribution, often (but not necessarily) a multivariate
normal. The rationale behind this regularization is that we are looking for an informative *latent space* of hidden variables,
whose expression to various degrees should create the range of "phenotypes" observed.

Thus, where GANs are designed solely for good generative performance, VAEs incorporate the additional aspiration to allow for
a meaningful representation of the domain modeled. Of course, what counts as a meaningful representation is open for debate.
...
respectively, and in the former case, take a look at the two-dimensional grid of artifacts.
...
submodules that are called, in order, from the `forward` method. Also contained in the module are the loss function and a
...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(image_size * 8, image_size * 4, kernel_size = 4, stride = 1, padding = 0),
            nn.BatchNorm2d(image_size * 4),
            nn.LeakyReLU(),
            # 8 * 8
            ...
            nn.LeakyReLU(),
            # 16 x 16
            nn.ConvTranspose2d(image_size * 2, image_size, kernel_size = 4, stride = 2, padding = 2),
            nn.BatchNorm2d(image_size),
            nn.LeakyReLU(),
            # 28 x 28
...
On the other hand, we can now visualize the latent space.

## Latent space

```{python}
kmnist_test = torchvision.datasets.KMNIST("data/kmnist", train = False, transform = transform)
test_loader = torch.utils.data.DataLoader(kmnist_test, batch_size=10000, shuffle = False, num_workers = 8)
...
encoded %>%
   ...
   coord_fixed(xlim = c(-8, -1.5), ylim = c(-8, -1.5))
```
...
Unfortunately, with a latent space dimensionality of 2, both dimensions end up strongly correlated, which is not something we
would want. That notwithstanding, let's see how we would explore transitions in that latent space.
...
for(i in 1:length(grid_x)){
  ...
  for(j in 1:length(grid_y)){
    z_sample <- torch$tensor(list(grid_x[i], grid_y[j]), dtype = torch$float32)$cuda()
...
Not too bad, though, for a latent dimensionality of 2 and a pretty complex dataset.
